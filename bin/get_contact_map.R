    ...
    for(j in seq_along(genes)) {
        ...
        if(!nrow(hybrid.dt) == 0) {
            ...
            if(opt$bin_size == 1) {
                ...
                binned.mat <- bin_matrix_rect(mat, bin.size = opt$bin_size)
            }
            ...
            fwrite(binned.dt, file = paste0(opt$output, ".", gene_A, "-", gene_B, ".", opt$bin_size, "_binned_map.tsv.gz"), sep = "\t")
