...
# Cluster jobs
if(!opt$shuffled_mfe) {
    sjob <- slurm_apply(.slurm_GetMFE, seq.df, jobname = sapply(strsplit(basename(opt$hybrids), "\\."), "[[", 1), nodes = opt$nodes, cpus_per_node = 1, slurm_options = list(time = "24:00:00"), submit = TRUE)
} else if(shuffled_mfe) {
    sjob <- slurm_apply(.slurm_AnalyseMFE, seq.df, jobname = sapply(strsplit(basename(opt$hybrids), "\\."), "[[", 1), nodes = opt$nodes, cpus_per_node = 1, slurm_options = list(time = "24:00:00"), submit = TRUE)
}
