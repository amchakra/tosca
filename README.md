...
- `hybrids` contains files that have the identified hybrids as TSV files:
    ...
    - `logs` contains logs produced during hybrid read mapping and identification logs
...
- `nonhybrids` contains those sequencing reads that did not contain a hybrid:
    - `*.nonhybrid.fastq.gz`
- `read_fate` contains Sankey visualisations of read fate at each pipeline step for each sample
